...
> 这个项目想展示的是：AI 产品经理怎么设计一条有证据、能回归、失败关闭、有人接管的高风险工作流——而不是让大模型自动审批。
...
**结论**：这个场景真正难的不是"识别发票"，而是识别不可靠的时候，怎么兜底、怎么审计、怎么让人接管。这是整个项目的设计出发点。
...
这个迭代和 RAG 效果差不一定要换更大的模型、改分块策略可能更有效是同一个道理：先找对数据源，再谈模型。二维码在这个场景里比任何 OCR 模型都准、便宜、快。
