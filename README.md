...
基于上海自仪的采购实习经历以及对真实报销审核流程的观察和访谈：
...
访谈了 3 位有报销审核经验的同学（2 位曾在实习中协助财务整理票据，1 位是创业公司的兼职财务）及我的母亲（资深财务总监），确认了几个关键事实：
...
**结论**：这个场景的核心不是"识别发票"，而是"在识别不可靠的前提下，设计一个能兜底、能审计、有人接管的工作流"。这也是本项目的设计出发点。
...
本项目经历了两个版本的迭代，核心发现是：**不是 OCR 不行，而是应该优先使用更可靠的结构化数据源**。
...
**关键洞察**：这个迭代和"RAG 效果差不一定要换更大的模型，改分块策略可能更有效"是同一个道理——AI 产品经理的价值不在于追求最强的模型，而在于找到最适合场景的数据源和技术组合。二维码就是这个场景里的"最优解"，它比任何 OCR 模型都更准确、更便宜、更快。
