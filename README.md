...
**已实现并有证据：** 文件哈希查重、二维码字段提取、批次/历史/近似规则、合成黄金集回归、公开结果脱敏、未验真禁止自动放行

**已设计但未实现：** 持久化人工复核队列和审核界面、节点级运行日志和P50/P95时延看板、OCR/视觉模型自动降级编排

**生产前必须补：** 税务权威验真/红冲/作废状态、企业身份/权限/数据加密/审计合规、只读历史台账接口、脱敏历史数据试点和R30阈值校准、真实人工复核流程与SLA
...
不要说："已经实现企业自动报销审批"、"准确率100%"、"已经接入豆包API"、"36/36代表线上不会漏拦"。
